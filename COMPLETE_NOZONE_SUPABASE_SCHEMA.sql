...
-- FIXED: Resolved ambiguous column reference issue
...
-- ============================================
-- 11. DIRECT MESSAGE READ RECEIPT SUPPORT (ADDED)
-- ============================================

-- Allow recipients to mark messages as read via SECURITY DEFINER function to avoid exposing broad update rights
...
GRANT EXECUTE ON FUNCTION mark_messages_read(UUID, UUID) TO authenticated, anon;

-- Optional: broaden RLS so recipients can update status to read (kept narrow)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'direct_messages' AND policyname = 'Recipients can mark messages read'
    ) THEN
        CREATE POLICY "Recipients can mark messages read" ON direct_messages
            FOR UPDATE USING (
                recipient_id = auth.uid()::uuid
            ) WITH CHECK (
                recipient_id = auth.uid()::uuid
